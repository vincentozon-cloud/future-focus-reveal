...
export default function Home() {
  ...
  return (
    // Updated main container to respond to dark mode implicitly
    <main className="min-h-screen relative overflow-hidden font-sans bg-[#fdf2f8] dark:bg-black transition-colors duration-500">
      
      {/* ========================================= */}
      {/* LIGHT MODE: Sakura Pink & Chalkboard Green */}
      {/* ========================================= */}
      <div className="fixed inset-0 z-0 block dark:hidden">
        <div 
          className="absolute inset-0 transition-opacity duration-1000"
          style={{
            background: `
              radial-gradient(at 0% 0%, rgba(252, 231, 243, 0.9) 0, transparent 50%), /* Light Sakura Pink */
              radial-gradient(at 50% 50%, rgba(255, 240, 245, 0.7) 0, transparent 60%), 
              radial-gradient(at 100% 0%, rgba(209, 250, 229, 0.8) 0, transparent 50%), /* Chalkboard Green */
              radial-gradient(at 100% 100%, rgba(252, 231, 243, 0.9) 0, transparent 50%),
              #fdf2f8
            `,
          }}
        />
        {/* Lighter texture overlay for clean light mode */}
        <div className="absolute inset-0 opacity-[0.03] bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]" />
      </div>

      {/* ========================================= */}
      {/* DARK MODE: Your Original Deep Aesthetic   */}
      {/* ========================================= */}
        ...
        <div className="relative z-10 animate-fadeIn duration-2000 min-h-screen">
          ...
          <div className="fixed top-6 right-6 z-70 flex items-center gap-3">
            ...
            <button 
              ...
              className="p-3 bg-white/10 backdrop-blur-xl border border-white/20 rounded-full text-slate-800 dark:text-white hover:bg-pink-600 transition-all shadow-xl"
            >
          ...
          <div className={`fixed left-0 top-0 h-full w-full md:w-100 z-60 transition-transform duration-1000 ease-in-out transform ${isLoginOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            <div className="h-full bg-white/80 dark:bg-black/40 backdrop-blur-3xl border-r border-slate-200 dark:border-white/10 p-8 flex flex-col justify-center shadow-2xl transition-colors duration-500">
          ...
          <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[90%] max-w-2xl z-50">
            <div className="bg-white/80 dark:bg-white/10 backdrop-blur-xl border border-slate-200 dark:border-white/20 rounded-2xl p-4 shadow-2xl flex items-center gap-4 text-slate-900 dark:text-white transition-colors duration-500">
